...
const pokemonId = props.initialPokemon._id || "";
...
const CREATE_POST_MUTATION = gql`
        mutation CreatePost($title: String!, $content: String!) {
            createPost(title: $title, content: $content) {
                id
                title
                content # Include content if you want to reuse it in the cache update
            }
        }

        mutation CreatePikachu($name: String!, type: [String!], abilities: [String!], height: Float!, weight: Float!, pokedexId: Int!) {
            createPokemon(
                input: {
                    name: $name,
                    type: $type,
...
const { executeMutation: createPokemonMutation } =
...
async function createPokemon() {
    try {
        const result = await createPokemonMutation({
        ...
        if (result.error) {
            ...
            // Optional: Manually update the cache after mutation if using 'cacheExchange'
            // For more robust cache updates, consider using 'graphcache'
            if (result.data && result.data.createPost) {
                // This is a basic example and might not work for complex scenarios
                // For 'cacheExchange', simply re-executing the query is often easier
                // Or for complex scenarios, 'graphcache' is highly recommended.
                // If you were using a more advanced cache like `graphcache`,
                // it would often handle these updates automatically based on `id`s.
            }
        }
    } catch (err) {
